	...
	function getsp2dQuery($kueri)
		{
			...
			if (isset($kueri)) {
				$query = $this->db->query("SELECT *
								from (
							SELECT 
							(SELECT no_uji from trduji where no_sp2d=a.no_sp2d)as no_uji,no_sp2d,
							nm_skpd,
							...
							)z where no_uji is not null and (no_sp2d like '%$kueri%' OR penerima like '%$kueri%' OR npwp like '%$kueri%') order by CAST(SUBSTRING(no_sp2d,0,LEN(no_sp2d)-7) as int) DESC");
			}else{
				$query = $this->db->query("SELECT TOP 10 *
							...
							from trhsp2d a where (sp2d_batal is null OR sp2d_batal =0)  
							and ((jns_spp=6 and jenis_beban=6 ) OR jns_spp = 5)
							)z where no_uji is not null order by CAST(SUBSTRING(no_sp2d,0,LEN(no_sp2d)-7) as int) DESC");
